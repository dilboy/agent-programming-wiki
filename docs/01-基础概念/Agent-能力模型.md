...
| 反模式 | 问题 | 影响 | 修复方案 |
|--------|------|------|---------|
| **能力过度承诺** | 声称 Agent 能做 L4/L5 的事，实际只有 L1 能力 | 用户失望、信任崩塌 | 诚实标注能力层级，逐步提升 |
| **跳级开发** | 直接从 L1 跳到 L4，跳过中间层级 | 基础不牢、频繁失败 | 逐层构建，每层稳定后再升级 |
| **忽视降级策略** | Agent 在所有场景都用最高能力模式 | 简单任务延迟高、成本高 | 实现能力降级，简单任务走快速路径 |
| **静态能力** | Agent 能力不随反馈迭代 | 长期停滞、无法适应新场景 | 建立评估闭环，持续优化 |
| **边界模糊** | 不清楚 Agent 能做什么、不能做什么 | 用户误用、系统失败 | 清晰的能力声明和错误提示 |
