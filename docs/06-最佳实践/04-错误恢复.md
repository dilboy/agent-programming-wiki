...
| 维度 | 方案A | 方案B | 建议 |
|------|-------|-------|------|
| **恢复方式** | 自动重试（指数退避 + 断路器） | 降级到备用方案（弱模型/缓存/模板） | 瞬时故障（网络抖动、Rate Limit）优先重试；持续故障（服务不可用、模型降级）切换备用方案 |
| **干预模式** | 全自动恢复（无需人工介入） | 人工确认后恢复 | 低风险操作（信息查询、内容生成）可全自动恢复；高风险操作（涉及数据修改）需人工确认后继续 |
| **失败策略** | 快速失败（尽早检测，立即报错） | 优雅降级（返回简化结果，不中断服务） | 对结果准确性要求极高的场景（医疗、金融）应快速失败；面向用户的体验型服务应优雅降级保可用性 |
| **重试范围** | 仅重试失败的单个步骤 | 回滚并重试整个子流程 | 步骤间无状态依赖时只重试失败步骤；步骤间有副作用且需要一致性时回滚整个子流程 |
| **回滚机制** | 补偿事务（记录逆操作，失败时逆序执行） | 检查点恢复（保存快照，回滚到安全点） | 写操作密集的分布式流程用补偿事务；长时间运行的单体任务用检查点恢复更简单 |
| **错误信息** | 详细技术信息返回调用方 | 用户友好的通用提示 | 面向开发者的 API 返回详细错误码和上下文；面向终端用户的界面映射为友好提示并提供操作建议 |
