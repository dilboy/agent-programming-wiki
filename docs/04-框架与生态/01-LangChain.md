...
选择 LangChain 的核心权衡是**生态丰富度 vs 框架复杂度**。

### LangChain vs 原生 SDK vs 其他框架

| 维度 | LangChain | 原生 OpenAI/Anthropic SDK | LangGraph | CrewAI |
|------|-----------|--------------------------|-----------|--------|
| 学习曲线 | 陡峭（抽象层多） | 平缓 | 更陡峭 | 平缓 |
| 生态丰富度 | 极高 | 低 | 高（继承 LangChain） | 中 |
| 适合场景 | RAG、工具集成 | 简单 LLM 调用 | 复杂有状态工作流 | 角色驱动团队 |
| 版本稳定性 | 低（API 频繁变化） | 高 | 中 | 中 |
| 性能开销 | 中（抽象层） | 低 | 中 | 低 |

### 抽象层级的取舍

- **高层抽象（Agent、Chain）**：开发速度快，但黑盒化严重，调试困难，性能开销大
- **低层抽象（LCEL 管道）**：灵活性高，可组合性强，但需要更多样板代码
- **直接调用 SDK**：最简单、性能最好，但失去跨模型切换和工具集成能力
- **经验法则**：先用 LCEL 管道起步，只在需要动态决策时引入 Agent；不要因为 LangChain 有 Agent 就一定用 Agent

### 版本锁定 vs 持续升级

- LangChain 的 API **变化极快**——0.1 到 0.2 的迁移涉及大量 breaking change
- 锁定版本可保证稳定性，但会错过性能优化和安全补丁
- **折中方案**：锁定主版本（如 `langchain>=0.2,<0.3`），定期评估升级收益

### 何时选择 LangChain

- 需要**快速集成多种 LLM 提供商**（OpenAI、Anthropic、Google 等）
- 需要**RAG、工具调用、记忆管理**等标准组件
- 团队希望**避免从零构建基础设施**
- 需要**LangSmith 可观测性**支持

### 何时避免 LangChain

- 只需要**简单的 LLM 调用**——原生 SDK 更轻量
- 对**性能极其敏感**——抽象层引入额外开销
- 需要**精细控制每个执行步骤**——框架的抽象可能遮蔽关键细节
- 项目**生命周期很长且需要稳定性**——API 频繁变化会增加维护负担
